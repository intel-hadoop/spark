...
class StorageLevel private(
    ...
    private var _usePersistentMemory: Boolean,
  ...
  def usePersistentMemory: Boolean = _usePersistentMemory
  ...
  if (usePersistentMemory) {
    require(!deserialized, "Persistent memory storage level does not support deserialized storage")
  }

  private[spark] def memoryMode: MemoryMode = {
    if (useOffHeap) {
      MemoryMode.OFF_HEAP
    } else if (usePersistentMemory) {
      MemoryMode.PERSISTENT_MEMORY
    } else {
      MemoryMode.ON_HEAP
    }
  }
  ...
  override def clone(): StorageLevel = {
    new StorageLevel(useDisk, useMemory, useOffHeap, usePersistentMemory, deserialized, replication)
  }
  ...
  override def equals(other: Any): Boolean = other match {
    case s: StorageLevel =>
      ...
      s.usePersistentMemory == usePersistentMemory &&
  ...
  override def readExternal(in: ObjectInput): Unit = Utils.tryOrIOException {
    ...
    _usePersistentMemory = (flags & 2) != 0
  ...
  override def toString: String = {
    ...
    val persistentMemory = if (usePersistentMemory) "persistentMemory" else ""
    val deserialize = if (deserialized) "deserialized" else ""

    val output =
      Seq(disk, memory, heap, persistentMemory, deserialize, s"$replication replicas")
        .filter(_.nonEmpty)
...
object StorageLevel {
  ...
  val PERSISTENT_MEMORY = new StorageLevel(true, true, false, true, false, 1)
  ...
  def fromString(s: String): StorageLevel = s match {
    ...
    case "PERSISTENT_MEMORY" => PERSISTENT_MEMORY
  ...
  def apply(
    ...
    getCachedStorageLevel(new StorageLevel(useDisk, useMemory, useOffHeap, usePersistentMemory,
      deserialized, replication))
  }

  /**
   * :: DeveloperApi ::
   * Create a new StorageLevel object without setting use usePersistentMemory.
   */
  @DeveloperApi
  def apply(
      useDisk: Boolean,
  ...
  /**
   * :: DeveloperApi ::
   * Create a new StorageLevel object without setting useOffHeap and usePersistentMemory.
   */
  @DeveloperApi
  def apply(
      useDisk: Boolean,
    ...
    getCachedStorageLevel(
      new StorageLevel(useDisk, useMemory, false, false, deserialized, replication))
  }
